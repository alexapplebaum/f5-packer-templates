...
# Changing password for packeruser.
...
tmsh modify auth user packeruser password TempPaSsWd%4353

tmsh save /sys config

# echo "Setting Password via Rest: $(date)"
# curl -sk -u admin: -H "Content-Type: application/json" -X PATCH https://localhost:8100/mgmt/tm/auth/user/packeruser -d '{ "password": "TempPaSsWd%4354" }'
...
#  root     20123  4272  3 13:26 ?        Ss     0:05 sshd: packeruser@notty
PACKER_SSH_PID=$(ps -ef | grep 'sshd: packeruser@notty' | grep -v grep | awk '{print $2}')
